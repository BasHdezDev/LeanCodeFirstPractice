...
class PaymentPlan:
    ...
    def payment_plan(self, credit_card: CreditCard, installments: int) -> list:
        """Creates a payment plan"""
        payment_value: float = round(credit_card.monthly_payment(self.purchase_amount, installments), 2)
        ...
        if installments == 1:
            ...
            row = [1, self.card_number, self.purchase_date, self.purchase_amount, self.payment_date,
                   self.payment_amount, 0.0, self.capital_amount, 0.0]
            ...
            for payment in range(1, installments + 1):
                ...
                row: list = [payment_number, self.card_number, self.purchase_date, self.purchase_amount,
                             self.payment_date,
